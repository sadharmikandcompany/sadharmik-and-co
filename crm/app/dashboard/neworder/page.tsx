...
import { Plus, X, ExternalLink, ChevronDown, Edit, Download, XCircle, Truck, CheckCircle, MoreVertical, Users, Printer, UserCheck, AlertTriangle, DollarSign, Trash2, Star, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight } from "lucide-react"
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
import { generateOrderInvoice, generateBulkOrderInvoices, generateThermalReceipt, generateBulkThermalReceipts } from "@/lib/invoice-generator"
import { PrinterSettingsDialog } from "@/components/printer-settings-dialog"
import { printThermalReceipts, isThermalPrintEnabled } from "@/lib/thermal-print"
...
export default function OrdersPage() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      // Fetch customer and retailer orders (exclude distributor orders)
      ...
      const [customersData, distributorsData, retailersData, deliveryPartnersData, serviceableDistributorsResult, routeAssignmentsData] = await Promise.all([
        ...
        fetchInBatches<any>(
          "customers",
          "id, first_name, last_name, mobile_primary, mobile_secondary_1, mobile_secondary_2, full_address, vip_number",
          customerIds
        ),
        ...
        fetchInBatches<any>(
          "route_assignments",
          "order_id, status, pickup_time, delivery_time, delivery_notes, collected_payment_method, collected_amount",
          orderIds,
          "order_id"
        ),
      ])
  ...
  const handleDownloadInvoice = async (orderId: string) => {
    try {
      ...
      // Fetch distributor company info by shipping pincode
      const companyInfo = orderData.shipping_pincode
  ...
  const handleDownloadThermalReceipt = async (orderId: string) => {
    try {
      ...
      // Fetch distributor company info by shipping pincode
      const distCompanyInfo = orderData.shipping_pincode
        ? await fetchDistributorCompanyInfo(orderData.shipping_pincode)
        : undefined

      // Build companyInfo for thermal print (subset of fields)
      const thermalCompanyInfo = distCompanyInfo ? {
        name: distCompanyInfo.name,
        address: distCompanyInfo.address,
        city: distCompanyInfo.city,
        pincode: distCompanyInfo.pincode,
        phone: distCompanyInfo.phone,
        gst: distCompanyInfo.gst,
      } : undefined

      // Check if direct thermal printing is enabled
      if (isThermalPrintEnabled()) {
        // Direct print to thermal printer with auto-cut
        const printData = {
          order: {
            order_number: orderData.order_number,
            invoice_number_gst: orderData.invoice_number_gst,
            ...
            is_priority: orderData.is_priority,
          ...
          customer: {
            ...
            last_name: customerData.last_name,
            mobile_primary: customerData.mobile_primary,
            mobile_secondary_1: customerData.mobile_secondary_1,
            vip_number: customerData.vip_number,
        ...
        const result = await printThermalReceipts([printData])
        if (result.success) {
          toast.success("Receipt printed successfully with auto-cut!")
        } else {
      ...
      const invoiceData = {
        ...
        companyInfo: distCompanyInfo,
      }

      // Generate payment QR URL for pending payments
      let paymentQrUrl: string | undefined = undefined
      if (orderData.payment_status === 'pending') {
        try {
          const paymentResponse = await fetch('/api/easebuzz/generate-link', {
            method: 'POST',
          ...
          if (paymentData.success && paymentData.paymentUrl) {
            paymentQrUrl = paymentData.paymentUrl
          }
        } catch (paymentError) {
  ...
  const handleBulkThermalPrint = async () => {
    ...
    try {
      ...
      const isDirectPrint = isThermalPrintEnabled()
      toast.loading(`${isDirectPrint ? 'Printing' : 'Generating'} ${selectedOrders.size} thermal receipts...`)

      const directPrintData: Array<{
        order: any
        customer: any
        items: any[]
        companyInfo?: any
      ...
      for (const orderId of Array.from(selectedOrders)) {
        try {
          ...
          // Fetch distributor company info by shipping pincode
          const distInfo = orderData.shipping_pincode
            ? await fetchDistributorCompanyInfo(orderData.shipping_pincode)
            : undefined

          const bulkThermalCompanyInfo = distInfo ? {
            name: distInfo.name,
            address: distInfo.address,
            city: distInfo.city,
            pincode: distInfo.pincode,
            phone: distInfo.phone,
            gst: distInfo.gst,
          } : undefined

          if (isDirectPrint) {
            // Prepare data for direct thermal printing
            directPrintData.push({
              order: {
                order_number: orderData.order_number,
                invoice_number_gst: orderData.invoice_number_gst,
                invoice_number_non_gst: orderData.invoice_number_non_gst,
              ...
              companyInfo: bulkThermalCompanyInfo,
            })
          } else {
            // Generate payment QR URL for pending payments (only for PDF)
            let paymentQrUrl: string | undefined = undefined
            if (orderData.payment_status === 'pending') {
              try {
                const paymentResponse = await fetch('/api/easebuzz/generate-link', {
            ...
            // Structure the invoice data for PDF
            thermalData.push({
              data: {
                order: {
                  id: orderData.id,
      ...
      if (isDirectPrint) {
        // Direct print to thermal printer with auto-cut
        if (directPrintData.length === 0) {
          toast.error("Failed to fetch data for all selected orders")
          setBulkThermalPrintLoading(false)
          return
        }

        const result = await printThermalReceipts(directPrintData)
        if (result.success) {
          if (failedOrders.length > 0) {
            toast.warning(
              `Printed ${directPrintData.length} receipts with auto-cut. Failed: ${failedOrders.join(", ")}`
            )
          } else {
            toast.success(`Successfully printed ${directPrintData.length} receipts with auto-cut!`)
          }
        } else {
          toast.error(result.error || "Failed to print receipts")
        }
      } else {
        // Fallback to PDF generation
  ...
  const getRowColorClasses = (order: Order) => {
    ...
    // and orders-v2/orders-table.tsx for the identical fix on those pages).
  ...
  const exportColumns = [
    ...
    { header: 'Delivery Proof URL', dataKey: 'Delivery Proof URL' },
