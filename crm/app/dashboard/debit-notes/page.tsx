...
export default function DebitNotesPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      setDebitNotes(debitNotes.filter((note) => note.id !== id))
  ...
  const handleMarkApproved = async (note: DebitNote) => {
    try {
      const { error } = await supabase
        .from("debit_notes")
      ...
      toast.success("Debit note marked as approved")
    } catch (error) {
  ...
  const filteredNotes = debitNotes.filter((note) => {
    const matchesSearch =
      ...
      (note.bill_number?.toLowerCase() || "").includes(searchQuery.toLowerCase())
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl md:text-2xl font-bold tracking-tight">Debit Notes</h1>
            <p className="text-sm text-muted-foreground">
              Manage debit notes issued to customers and vendors
            </p>
          </div>
        </div>
      ...
      <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-5">
        <Card className="h-full">
          <CardHeader>
            <CardDescription>Total Debit Notes</CardDescription>
          ...
          <CardContent className="text-xs text-muted-foreground">{hasActiveFilters ? "Across current filters" : "All debit notes"}</CardContent>
        </Card>
        ...
        <CardHeader>
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
            <div className="flex items-start gap-3">
              ...
              <div>
                <CardTitle className="text-base">Debit Note Records</CardTitle>
                <CardDescription className="mt-0.5">Search and manage debit notes</CardDescription>
              </div>
            </div>
          ...
          <div className="mt-4 flex flex-col sm:flex-row sm:flex-wrap items-stretch sm:items-center gap-2">
            <div className="relative w-full sm:w-auto">
              ...
              <Input
                placeholder="Search by note number, party name, bill..."
                value={searchQuery}
            ...
            <Button variant="outline" onClick={fetchDebitNotes}>
          ...
          <Table className="min-w-[1150px] w-full">
            <TableHeader>
              <TableRow className="bg-muted/40 hover:bg-muted/40">
                ...
                <TableHead className="font-semibold uppercase tracking-wider text-[11px]">Bill #</TableHead>
              ...
              {loading ? (
                ...
                filteredNotes.map((note) => (
                  <TableRow key={note.id} className="transition-colors hover:bg-muted/30">
                    ...
                    <TableCell>{note.bill_number || "-"}</TableCell>
                    <TableCell>
                    ...
                    <TableCell className="text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem
                            onClick={() => router.push(`/dashboard/debit-notes/${note.id}`)}
                          >
                            <Eye className="h-4 w-4 mr-2" />
                            View / Print
                          </DropdownMenuItem>
                          <DropdownMenuItem
                            onClick={() => router.push(`/dashboard/debit-notes/${note.id}/edit`)}
                          >
